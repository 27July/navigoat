...
const CONFIG = {
  ...
  API_ENDPOINT_PROGRESSIVE: 'http://localhost:3000/api/simplify-progressive',
...
/**
 * Process elements with progressive endpoint (for faster initial response)
 */
async function processElementsProgressive(elements, pageUrl, pageTitle, chunkSize = 5) {
  try {
    const apiEndpoint = CONFIG.API_ENDPOINT_PROGRESSIVE;
    console.log('[CogniClear] Calling progressive API:', apiEndpoint);
    ...
    console.log(`[CogniClear] Progressive API response in ${processingTime}ms`);
    ...
    console.error('[CogniClear] Error calling progressive API:', error);
    throw error;
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'PROCESS_ELEMENTS_PROGRESSIVE') {
    const { elements, pageUrl, pageTitle, chunkSize } = message.payload;
    
    // Process first chunk with progressive endpoint
    processElementsProgressive(elements, pageUrl, pageTitle, chunkSize)
      .then(result => {
        sendResponse(result);
      })
      .catch(error => {
        console.error('[CogniClear] Progressive processing error:', error);
        sendResponse({
          success: false,
          error: error.message,
          simplified: fallbackCategorization(elements)
